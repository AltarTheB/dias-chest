...
class Gameboard:
    ...
    def __generate_board(self, size):
        """Генерирует исходное состояние доски с фигурами."""
    ...
    def move(self, current_position, destination):
        """Перемещает фигуру из текущей позиции в место назначения на доске."""
        ...
        if self.last_move == self.board[cur_y][cur_x].color:
            return Result(False, 'Сейчас не твоя очередь!')
        ...
        if dst_is_legal_move and type(self.board[dst_y][dst_x]) not in (LightPiece, DarkPiece):
            ...
            return Result(False, 'Этот ход невозможен!')

    def __ensure_valid_board(self, board):
        """Обеспечивает, чтобы доска была квадратной, а фигуры расставлены на черных квадратах."""

        board_size = len(board)

        for y, row in enumerate(board):
            if len(row) != board_size:
                raise ValueError('нельзя использовать неквадратную доску')

            for x, square in enumerate(row):
                if type(square) in (LightPiece, DarkPiece):
                    legal_square = self.__is_legal_square(x, y)
                    if not legal_square:
                        raise ValueError('не могу поставить фигуру на белом квадрате')

        return board

    def __is_legal_square(self, x, y):
        """Проверяет, черный ли данный квадрат."""
    ...
    def __is_move_within_bounds_of_board(self, dst_x, dst_y):
        """Проверяет, находится ли данный ход в пределах доски."""
